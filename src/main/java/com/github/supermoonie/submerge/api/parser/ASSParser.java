...
import com.github.supermoonie.submerge.api.exception.InvalidAssSubException;
import com.github.supermoonie.submerge.api.subtitle.ass.*;
...
public class ASSParser extends BaseParser<ASSSub> {
        ...
        for (int i = 0; i < eventsFormat.length; i++) {
            ...
            try {
                switch (property) {
                    ...
                    default:
                        try {
                            BeanUtils.setProperty(events, property, value);
    ...
    private static V4Style parseV4Style(String[] styleFormat, String[] styleLine, int lineIndex)
        ...
        for (int i = 0; i < styleFormat.length; i++) {
            ...
            Object value = styleLine[i].trim();

            if (property.toLowerCase().contains("colour")) {
                // Colors can be number (bgr) or string (&HBBGGRR or &HANGAR)
                value = ColorUtils.HAABBGGRRToAlphaColor(value.toString());
            }
            try {
                BeanUtils.setProperty(style, property, value);
            } catch (IllegalAccessException | InvocationTargetException e) {
                throw new InvalidAssSubException(e);
            }
        }
        ...
        while (line != null && !line.startsWith("[")) {
            ...
            if (!line.startsWith(COMMENTS_MARK)) {
                ...
                if (split.length > 1) {
                    ...
                    try {
                        BeanUtils.setProperty(scriptInfo, property, value);
                    } catch (IllegalAccessException | InvocationTargetException e) {
                        throw new InvalidAssSubException("Script info : " + e.getMessage(), e);
                    }
